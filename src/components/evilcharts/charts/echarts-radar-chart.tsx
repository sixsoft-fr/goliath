...
function createTooltipFormatter(ctx: OptionBuildContext) {
  ...
  return (params: unknown): string => {
    ...
    const dimmed =
      selectedDataKey != null && selectedDataKey !== key ? " opacity-30" : ""
...
export function EChartsRadarChart<TData extends Record<string, unknown>>({
  ...
  const seriesKeys = useMemo(
    () => radars.map((radar) => radar.dataKey),
    [radars]
  )
  ...
  const buildOption = useCallback((): EChartsOption => {
    ...
    for (const key of seriesKeys) {
      for (const row of data)
        indicatorMax = Math.max(indicatorMax, Number(row[key]) || 0)
    }
  ...
  useEffect(() => {
    ...
    const pushReveal = () => {
      ...
      const series =
        (merged.series as unknown as RevealSeries[] | undefined) ?? []
    ...
    if (
      ...
      const idx = Math.min(
        Math.max(tooltipSlot.defaultIndex, 0),
        seriesKeys.length - 1
      )
