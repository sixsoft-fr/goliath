...
import {
  type Resource,
  type TableQueries,
} from "@/modules/core/service/api.types"
...
export class Service {
  ...
  async get<T>(): Promise<KyResponse<Resource<T>>> {
    const response = await api.get<Resource<T>>(
      `/${this.resource}/${this.identifier}`
    )
    return response
  }

  async list<T>(
    queryElements: TableQueries
  ): Promise<KyResponse<PaginatedResponse<T>>> {
